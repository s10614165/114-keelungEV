...
const sectionMap = {
    "核准通過證明文件": ["teenPassProof"],
    "車行銀行存摺封面": ["teenBank"],
    "領據(附件7)": ["teenAffidavit"]
};
const sections = {
    teenPassProof: document.querySelector('section:has(#file-teenPassProof)'),
    teenBank: document.querySelector('section:has(#file-teenBank)'),
    teenAffidavit: document.querySelector('section:has(#file-teenAffidavit)')
};
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // 監聽下拉選單變更事件
    select.addEventListener("change", updateVisibleSections);
    ...
    // 表單驗證
    const url = ""; // Apps Script Web App URL
    ...
    document.getElementById("subsidyForm").addEventListener("submit", async function (e) {
      ...
      try {
          ...
          if (text.trim().toLowerCase() === 'success'){
            ...
            window.location.href = 'teenSuccess.html';
          }
          else {
            alert("提交失敗：" + text);
          }
...
function updateVisibleSections() {
    ...
    if (sectionMap[selected] && sectionMap[selected].includes(key)) {
